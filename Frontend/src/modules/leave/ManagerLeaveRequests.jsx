  ...
  return (
    <div className="page-container">
      <div className="page-header">
        <h2><CalendarClock size={24} style={{ display: 'inline', marginRight: '10px' }}/> Leave Requests</h2>
      </div>
      ...
      <div className="glass-panel p-4">
        {leaves.length === 0 ? (
          <p className="text-gray-400">No pending leave requests for your team.</p>
        ) : (
              ...
              <tbody>
                {leaves.map(l => (
                  <tr key={l.id} style={{ borderBottom: '1px solid #e2e8f0' }}>
                    ...
                    <td style={{ padding: '16px' }}>{new Date(l.createdAt).toLocaleDateString()}</td>
                    <td style={{ padding: '16px' }}><span className="status-badge status-pending">{l.status}</span></td>
                    <td style={{ padding: '16px' }}>
                      <button onClick={() => handleStatusChange(l.id, 'MANAGER_APPROVED')} className="btn btn-primary" style={{ marginRight: '8px', padding: '6px 12px' }}>Approve</button>
