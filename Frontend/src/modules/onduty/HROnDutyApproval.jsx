...
const HROnDutyApproval = () => {
  ...
  const fetchRequests = async () => {
    try {
      ...
      setRequests(res.data?.filter(o => o.status === 'MANAGER_APPROVED') || []);
    } catch (err) {
  ...
  return (
    <div className="page-container">
      <div className="page-header">
        <h2><Briefcase size={24} style={{ display: 'inline', marginRight: '10px' }}/> On Duty Approval</h2>
      </div>
      ...
      <div className="glass-panel p-4">
        {requests.length === 0 ? (
          <p className="text-gray-400">No On Duty requests pending HR approval.</p>
        ) : (
              ...
              <tbody>
                {requests.map(r => (
                  <tr key={r.id} style={{ borderBottom: '1px solid #e2e8f0' }}>
                    ...
                    <td style={{ padding: '16px' }}><span className="status-badge status-approved">{r.status}</span></td>
                    <td style={{ padding: '16px' }}>
                      <button onClick={() => handleStatusChange(r.id, 'APPROVED')} className="btn btn-primary" style={{ marginRight: '8px', padding: '6px 12px' }}>Approve</button>
...
export default HROnDutyApproval;
